...
test_data_path = os.path.join(test_folder_path, 'testdata.csv')
...
with open(ingested_file_path) as f:
...
if not all_files_exist:
    print("Have new data => Ingest new datasets from {}".format(input_folder_path))
    subprocess.call(['python', 'ingestion.py'])
else:
    print("No new ingested data. Exit the process")
    exit(0)
...
print('Lastest F1 score: {}, F1 score on newly ingested data: {}'.format(latest_score, new_score))
...
if new_score >= latest_score:
    ...
    print('Model drift => Retrain and Re-deploy model')
